<template>
  <div>
    <TerminEditor v-if="currentMeeting" :value="currentMeeting" @update="reload"/>

    <div v-for="meeting in meetings" :key="meeting.id">
      <button type="button" class="close ml-3 mt-4 float-right" aria-label="Close">
      ...
      <button type="button" class="close ml-3 mt-4 float-right" aria-label="Edit" @click="currentMeeting=meeting">
...
export default {
  ...
  methods: {
    async reload() {
      ...
      this.meetings = await api.getAllMeetings();
    }
  },
  props: {},
  async mounted() {
